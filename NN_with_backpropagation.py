...
#since mnist images are 128x128, it resize the image to 32x32 and then store pixel values 
def load_images(folder):
	file=open("Input_to_NNN.txt","w+")
...
#				print("ORIGINAL w : ",w," h : ",h)
				img0=cv.resize(img,(0,0),fx=1/4,fy=1/4)
				w,h=np.shape(img0)
#				print("ORIGINAL w : ",w," h : ",h)
				file.write(filename)
				for i in range(w):
...
flag=False
for f in os.listdir(full_path):
	if(f=="Input_to_NNN.txt"):
		flag=True
		break

if(flag==False):
	home = os.path.join(full_path,"Img")
	#dict['label']=[images]
	#print(home)
	load_images(home)
...
print("Input Image  : ")
print("w : ",w," h : ",h)
inputImage=cv.resize(inputImage,(0,0),fx=1/4,fy=1/4)
w,h=np.shape(inputImage)
print("Input Image  : ")
print("w : ",w," h : ",h)
for i in range(w):
...
print("INNNER WEIGHT : ",len(innerWeights))
for i in range(len(outerWeights)):
	print("OUTER WEIGHT["+str(i)+"] : ",len(outerWeights[i]))
for i in range(hidden):
	s=hiddenbias
	for j in range(i_p):
		s=s+innerWeights[i][j]*inputLayer[j]
...
print("MAX : ",max(outputLayer))
print("OUTPUT : ",OutPut[outputLayer.index(max(outputLayer))])